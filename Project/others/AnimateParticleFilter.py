...
def AnimateParticleFilter(map, measurements, num_particles, estimates, high_noise=False):
    # true landmark locations
    landmarks = np.load(map, allow_pickle=True)

    # Remove .npy extension from estimates and concatenate with num_particles
    if estimates.endswith('.npy'):
        estimates = estimates[:-4]  # Remove last 4 characters, which should be '.npy'
    file_name = f"{estimates}_{num_particles}.npy"
    ...
    particles = []
    initial_pose = measurements[0]
    initial_weight = 1.0 / num_particles
    for _ in range(num_particles):
        particles.append(
            Particle(Robot(initial_pose[0], initial_pose[1], initial_pose[2]), initial_weight))
